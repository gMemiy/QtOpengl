...
void Scene::paintGL() // отрисовка сцены
{
    ...
    DrawBackGround(_backGround); // звёзды

    glBindTexture(GL_TEXTURE_2D, _spot); // биндим текстуру для частиц 1 раз (т.к. у всех частиц одна текстура)

    _fw.Draw(); // фейерверк

    _cloud.Draw(); // облака

    DrawBackGround(_foreGround); // деревья на переднем фоне
}
...
{
    _fw.Push(ScreenToWorld(pe->pos()), 1); // 1 - количество дочерних фейерверков. После взрыва первой ракеты будет создан 1 дочерний фейерверк
}
...
void Scene::InitFireWork() // тут нахардкожено, вместо работы с фалом настроек
{
    ...
    // натраиваем первую ракету (которая летит от мышки)
    BulletType first;
